...
layui.use(['jquery','layer', 'table','form','laydate'], function() {
    ...
    table.on('tool(test)', function (obj) {//注：tool 是工具条事件名，test 是 table
        ...
        var layEvent = obj.event;//获得 lay-event 对应的值（也可以是表头的 event 参数对 应的值）
            ...
            case 'delBatchOrders':
                ...
                if(data.length != 0){
                    ...
                    if(checkDelIf){
                        ...
                        layer.confirm('真的需要删除订单数据？', function(index){
    ...
    //修改订单显示状态
    function updOrdersFlag(id,obj){ $.post( "/orders/updT", //调用的是base系列的方法，只需要改mapper.xml文件
        {"id":id,"flag":"0"},
        function (data){ if(data === 'success'){
            layer.msg("订单数据删除成功！",{icon: 1,time:2000,anim: 4,shade:0.5});
            obj.del();//删除对应行（tr）的DOM结构，并更新缓存
            }else{ layer.msg("订单数据删除失败！",
            {icon: 2,time:2000,anim: 4,shade:0.5});
        }
            },"text" ).error(function (){
